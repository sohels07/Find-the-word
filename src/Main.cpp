#include <bits/stdc++.h>
#include "puzzle/Response.h"
#include "puzzle/Request.h"
#include "puzzle/PuzzleSolver.cpp"

using namespace std;

void testResultFound1() {
    string word = "netapp";
    ...
    request.setPuzzle(puzzle);
    request.setPattern(word);

    PuzzleSolver puzzleSolver = PuzzleSolver();
    Response response = puzzleSolver.findWord(request);

    if(response.getX() != -1) {
        cout << "x=" << response.getX() << ", y=" << response.getY() << ", count=" << response.getCount() << "\n\n";
    } else {
        cout << "No Result Found\n\n";
    }
}

void testResultFound2() {
    string word = "containers";
